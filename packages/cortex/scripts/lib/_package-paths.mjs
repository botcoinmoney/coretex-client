/**
 * Path resolution for the runtime scripts SHIPPED INSIDE the @botcoin/cortex
 * package (packages/cortex/scripts/). Counterpart of the repo-level
 * scripts/_repo-root.mjs, but it must work in BOTH layouts:
 *
 *   1. repo checkout:        <repo>/packages/cortex/scripts/lib/_package-paths.mjs
 *   2. node_modules install: <proj>/node_modules/@botcoin/cortex/scripts/lib/_package-paths.mjs
 *
 ...
 *      (detected via the workspace root package.json name `botcoin-cortex`).
...
const here = dirname(fileURLToPath(import.meta.url)); // packages/cortex/scripts/lib
export const packageRoot = resolve(here, '../..');     // packages/cortex
...
function detectRepoRoot() {
  ...
  try {
    ...
    if (existsSync(pkgPath)) {
      ...
      if (pkg.name === 'botcoin-cortex') return candidate;
    }
  } catch {
