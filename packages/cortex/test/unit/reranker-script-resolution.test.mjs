...
/** Build a simulated @botcoin/cortex package at `pkgRoot` and return the URL
...
describe('reranker script path resolution (package-root walk-up)', () => {
  test('repo-checkout layout: <repo>/packages/cortex/dist/eval → <repo>/packages/cortex', () => withTmpDir((dir) => {
    const pkgRoot = join(dir, 'repo', 'packages', 'cortex');
    // The workspace root has its OWN package.json with a different name — the
    // walk must stop at the @botcoin/cortex package, not the workspace root.
    mkdirSync(join(dir, 'repo'), { recursive: true });
    writeFileSync(join(dir, 'repo', 'package.json'), JSON.stringify({ name: 'botcoin-cortex' }));
  ...
  test('node_modules install layout: <proj>/node_modules/@botcoin/cortex/dist/eval → the installed package', () => withTmpDir((dir) => {
    const proj = join(dir, 'proj');
    mkdirSync(proj, { recursive: true });
    writeFileSync(join(proj, 'package.json'), JSON.stringify({ name: 'some-consumer-project' }));
    const pkgRoot = join(proj, 'node_modules', '@botcoin', 'cortex');
    const fromUrl = buildSimulatedPackage(pkgRoot);
    assert.equal(resolveCortexPackageRoot(fromUrl), pkgRoot);
  ...
  test('no @botcoin/cortex ancestor is a hard error (never silently spawn a wrong runner)', () => withTmpDir((dir) => {
