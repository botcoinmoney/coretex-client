...
"""Recover the public epoch law from confirmed chain data.

WHY THIS IS A SEPARATE MODULE AND NOT A LOOKUP AT THE CALL SITE. The single most common way to
get epoch verification wrong is to check a transition against a locally assumed rules version.
The rig lane makes that easy to do by accident, because both halves of its law are versioned in
ways that read like constants:

* the **epoch context** (``epochContextRoot``, ``coreVersionHash``) is set per epoch on the
  verifier and fixed before the epoch commit; the manifest addressed by ``epochContextRoot`` contains
...
for the policy. Neither needs the current chain state, which is exactly the property that makes
  replay of later public epochs possible on a chain that has since moved on.
...
from package constants. Using a local table as fallback would silently turn "we could not check
the price" into "the price was right".
"""
...
LAW_FORMAT = "coretex.rig-epoch-law/v1"
    ...
    return EpochLaw(
        ...
        sources={
            ...
            "policy": ({"source": ("verifier.activeCoreTexRulesVersion + getCoreTexPolicy"
                       ...
                       "is deliberately NOT substituted from a local table"),
        })
